...
@interface JobAllNotyViewController ()<UITableViewDataSource, UITableViewDelegate>
{
...
@implementation JobAllNotyViewController
...
- (void)viewDidLoad {
    ...
    allTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - 110) style:UITableViewStylePlain];
    allTable.dataSource = self;
    allTable.delegate = self;
    [allTable setBackgroundColor:[UIColor clearColor]];
    allTable.userInteractionEnabled=YES;
    [allTable setAllowsSelection:YES];
    if ([allTable respondsToSelector:@selector(setSeparatorInset:)]) {
        [allTable setSeparatorInset:UIEdgeInsetsZero];
    }
    [allTable setSeparatorStyle:UITableViewCellSeparatorStyleNone];
    [self.view addSubview:allTable];
}
