...
@interface JobChaViewController ()<UITableViewDataSource, UITableViewDelegate>
{
    UITableView *chaTable;
    NSMutableArray *chaArray;
}
@end

@implementation JobChaViewController

- (void)viewDidLoad {
    [super viewDidLoad];
   
    chaArray = [[NSMutableArray alloc] init];
    // Do any additional setup after loading the view.
    self.view.backgroundColor = [UIColor whiteColor];
    
    chaTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - 110) style:UITableViewStylePlain];
    chaTable.dataSource = self;
    chaTable.delegate = self;
    [chaTable setBackgroundColor:[UIColor clearColor]];
    chaTable.userInteractionEnabled=YES;
    [chaTable setAllowsSelection:YES];
    if ([chaTable respondsToSelector:@selector(setSeparatorInset:)]) {
        [chaTable setSeparatorInset:UIEdgeInsetsZero];
    }
    [chaTable setSeparatorStyle:UITableViewCellSeparatorStyleNone];
    [self.view addSubview:chaTable];
}
