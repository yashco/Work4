...
@interface JobRequestViewController ()<UITableViewDataSource, UITableViewDelegate>
{
...
- (void)viewDidLoad {
    ...
    requestTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - 110) style:UITableViewStylePlain];
    requestTable.dataSource = self;
    requestTable.delegate = self;
    [requestTable setBackgroundColor:[UIColor clearColor]];
    requestTable.userInteractionEnabled=YES;
    [requestTable setAllowsSelection:YES];
    if ([requestTable respondsToSelector:@selector(setSeparatorInset:)]) {
        [requestTable setSeparatorInset:UIEdgeInsetsZero];
    }
    [requestTable setSeparatorStyle:UITableViewCellSeparatorStyleNone];
